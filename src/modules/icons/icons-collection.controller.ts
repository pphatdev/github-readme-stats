...
export class IconsCollectionController {
    ...
    private static getFallbackColor(iconName: string, index: number): string {
        ...
        const paletteIndex =
            (parseInt(hash.slice(0, 8), 16) % IconsCollectionController.MULTI_ICON_COLOR_PALETTE.length);
        return IconsCollectionController.MULTI_ICON_COLOR_PALETTE[paletteIndex];
    }

    private static normalizeSize(
        value: unknown,
    ): keyof typeof IconsCollectionController.ICON_SIZE_PRESETS | null {
    ...
    private static applySvgColor(svgContent: string, color: string): string {
        ...
        if (result === beforeCurrentColor) {
            ...
            result = result.replace(
                /fill='(?:#000(?:000)?|black|rgb\(0\s*,\s*0\s*,\s*0\))'/gi,
                `fill='${color}'`,
            );
            result = result.replace(
                /stroke="(?:#000(?:000)?|black|rgb\(0\s*,\s*0\s*,\s*0\))"/gi,
                `stroke="${color}"`,
            );
            result = result.replace(
                /stroke='(?:#000(?:000)?|black|rgb\(0\s*,\s*0\s*,\s*0\))'/gi,
    ...
    private static buildInlineCollectionIconSvg(
        ...
        return svgContent.replace(/^\s*<svg[^>]*>/i, (svgTag) => {
            const attributes = svgTag
                .slice(4, -1)
            ...
            return `<svg${attributes ? ` ${attributes}` : ''}${classPart}${stylePart}${filterPart} x="${x}" y="${y}" width="${size}" height="${size}" overflow="visible" preserveAspectRatio="xMidYMid meet">`;
        });
    }
